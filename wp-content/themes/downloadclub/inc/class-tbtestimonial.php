...
class  CBXTestimonial {
	...
	public function __construct() {
		...
		add_filter( 'manage_testimonial_posts_columns', array(
			$this,
			'testimonial_columns_header'
		) ); // show or remove extra column
		add_action( 'manage_testimonial_posts_custom_column', array(
			$this,
			'testimonial_custom_column_row'
	...
	public function testimonial_columns_header( $columns ) {
		...
		//add cols
		$columns['cbxtestimonial_product']  = esc_html__( 'Product', 'downloadclub' );
		$columns['cbxtestimonial_featured'] = esc_html__( 'Featured', 'downloadclub' );
	...
	public function testimonial_row_actions_petition_listing( $actions, $post ) {
		if ( $post->post_type === 'testimonial' ) {
			...
			if ( $product_id > 0 ) {
				...
				$actions['all_reviews_public']       = '<a target="_blank" href="' . $testimonial_archive_link . '">' . esc_html__( 'All Reviews', 'downloadclub' ) . '</a>';
				$actions['all_reviews_admin']        = '<a href="' . admin_url( 'edit.php?post_type=testimonial&product_id=' . intval( $product_id ) ) . '">' . esc_html__( 'All Reviews(admin)', 'downloadclub' ) . '</a>';
				$actions['testimonial_edit_product'] = '<a target="_blank" href="' . get_edit_post_link( $product_id ) . '">' . esc_html__( 'Edit Product', 'downloadclub' ) . '</a>';
			}
		}
	...
	function cbrelatedpostajax_callback() {
		...
		if ( sizeof( $posts_array ) > 0 ) {
			...
			echo '<li>' . esc_html__( 'No search result found', 'downloadclub' ) . '</li>';
		}
