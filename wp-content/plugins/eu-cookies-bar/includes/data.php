...
class EU_COOKIES_BAR_Data {
	...
	public function __construct() {
		...
		if ( (int) $privacy_page > 0 ) {
			$privacy = get_post( $privacy_page )->post_content;
		} else {
			$privacy = '<header>
...
' . esc_html__( 'If we decide to change our privacy policy, we will post those changes on this page.', 'eu-cookies-bar' ) . '
		...
		$args         = array(
			...
			'strictly_necessary_family'                  => 'wp-settings,wordpress_logged_in,wp_woocommerce_session,woocommerce',
